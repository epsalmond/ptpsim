...
If BLE AP launch fails before Wi-Fi/PTP, `scripts/camera_ap_ptpip_probe_flow.sh` records `scripts/evidence/camera_ap_ble_session.sh --session-dir rce/sessions/laptop_ble_gps_<timestamp>` automatically. The evidence parser reads `session.log` and records `camera_ap_ble_launch=not_launched` when the function-launch write occurred but AP state never reached `0180/launched`.
