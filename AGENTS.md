...
If Ethernet is unavailable, use `scripts/camera_ap_ptpip_probe_flow.sh --temporary-wifi-internet`, optionally with `--restore-wifi-ssid SSID`. This intentionally lets Wi-Fi join the camera AP for the local PTP/IP probe and then restores the previous Wi-Fi SSID before the script exits. The lower-level `scripts/connect_camera_ap_wifi.sh --allow-wifi-internet-loss` only permits association; the combined flow owns restoration.
