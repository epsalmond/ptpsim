...
If Ethernet is unavailable, use `scripts/camera_ap_ptpip_probe_flow.sh --temporary-wifi-internet`, optionally with `--restore-wifi-ssid SSID`. This intentionally lets Wi-Fi join the camera AP for the local PTP/IP probe and then restores the previous Wi-Fi SSID before the script exits. The lower-level `scripts/connect_camera_ap_wifi.sh --allow-wifi-internet-loss` only permits association; the combined flow owns restoration. The combined flow must verify internet reachability before returning: it loops restore attempts in `04_restore_wifi.log`, accepts successful ping as proof even if `networksetup -getairportnetwork` is stale, and exits `9` if restoration cannot be verified.
...
The combined AP/PTP flow reads the camera LCD at transition points by default. If `scripts/read_camera_screen_state.sh` reports `camera_screen_state=unknown`, the flow stops; fix classifier/templates or LCD/iPhone alignment before retrying.
If BLE AP launch fails before Wi-Fi/PTP, record `scripts/evidence/camera_ap_ble_session.sh --session-dir rce/sessions/laptop_ble_gps_<timestamp>`. It parses `session.log` and records `camera_ap_ble_launch=not_launched` when the function-launch write occurred but AP state never reached `0180/launched`.
`scripts/ptpip_probe.sh` sends an reference app-shaped 82-byte Init_Command_Request by default: 16-byte GUID, four zero bytes, a fixed 26-byte UTF-16LE friendly-name field, and a 28-byte reference app tail. It keeps macOS route checks in shell and delegates packet construction, socket exchange, artifacts, and `summary.json` to `rce.tools.fuji_ble_gps.ptpip`. It validates generated tail lengths and supports `--guid HEX` for deterministic generated-identity tests and `--init-payload PATH` for replaying exact captured init payloads. It also supports `--open-session`, which sends raw PTP OpenSession transaction 1 after an init ack, `--get-prop HEX`, which sends PTP GetDevicePropValue after OpenSession, `--get-object-info HANDLE`, and `--get-thumb HANDLE`. `scripts/camera_ap_ptpip_probe_flow.sh` passes through `--ptpip-friendly-name`, `--ptpip-tail-profile`, `--ptpip-guid`, `--ptpip-init-payload`, `--ptpip-open-session`, `--ptpip-get-prop`, `--ptpip-get-object-info`, and `--ptpip-get-thumb`.
