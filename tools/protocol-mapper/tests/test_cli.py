...
def test_cli_decode_payload(capsys) -> None:
    rc = cli.main(["decode-payload", "d0d48a164c3fffb62100000000000000ea070501001a0e"])
    out = capsys.readouterr().out
    assert rc == 0
    assert "lat=37.81972" in out
    assert "utc=2026-05-01T00:26:14Z" in out


def test_cli_decode_payload_file(tmp_path, capsys) -> None:
    payload = tmp_path / "payload.bin"
    payload.write_bytes(bytes.fromhex("d0d48a164c3fffb62100000000000000ea070501001a0e"))

    rc = cli.main(["decode-payload", str(payload)])

    assert rc == 0
    assert "lon=-122.47861" in capsys.readouterr().out
