...
| `decode_init_command_response(packet)` | a typed `Acknowledged` or `Failed` standard PTP/IP init response, including the InitFail reason used with manifest-owned retry policy; consumers do not inspect packet-type literals or offsets |
