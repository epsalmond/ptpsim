...
| `transport_close(connection)` | the manifest-resolved frame for orderly command-transport teardown (Fuji `app`: the 8-byte sentinel), `None` when absent; malformed sentinel data is an error. Sending it does not by itself guarantee that the endpoint is immediately redialable. |
