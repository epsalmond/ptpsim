...
| `bleRequestMtu` | request ATT MTU `requestedMtu` before GATT traffic. If your platform has no request API (CoreBluetooth negotiates on its own), treat as a checkpoint: with no `minimumMtu`, succeed at any negotiated MTU; with one, fail when the negotiated MTU is below it. |
