...
| `selected_object_transfer(connection)` | typed lazy-gallery projection of the canonical `importObjects` per-handle preparation plus the existing chunk-read action; exposes the ObjectInfo payload index and manifest-owned u64 transfer-size/u32 chunk-size slots without requiring consumers to inspect nested action ASTs |
