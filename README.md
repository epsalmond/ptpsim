...
`sdcard-folder-and-dates` continues with `FujiVendor_9050` and `FujiVendor_9053`; the reference capture labels those as the active folder and capture-date list. `sdcard-object-handles` continues again with `FujiVendor_D620` and `FujiVendor_D621`, which the reference labels as object count and visible object handles.
Live laptop evidence reached `sdcard-folder-and-dates`, but `FujiVendor_9053` returned a large fixed-size payload whose only decoded UTF-16 text was `140_FUJI`, then `FujiVendor_D620` timed out. Treat this as a protocol mismatch to inspect before another AP run, not as a Wi-Fi failure.
